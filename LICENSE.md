# Proprietary License

**Copyright © 2025 HUMMBL Systems LLC. All Rights Reserved.**

---

## License Terms

This HUMMBL Unified Tier Framework (the "Framework") is proprietary intellectual property owned by HUMMBL Systems LLC ("HUMMBL"). By accessing, using, or referencing this Framework, you agree to the following terms:

---

## 1. Permitted Uses

### 1.1 Educational and Research Use
✅ **PERMITTED** with proper attribution:
- Academic research and study
- Educational instruction and learning
- Non-commercial analysis and critique
- Citation in academic publications
- Personal skill development

### 1.2 Personal Problem-Solving
✅ **PERMITTED** for individual use:
- Applying the Framework to personal challenges
- Using assessment methodologies for self-improvement
- Learning mental model applications
- Individual professional development

### 1.3 Attribution Requirements
When using this Framework, you MUST:
1. Credit HUMMBL Systems LLC for proprietary components
2. Credit DeepSeek AI for applicable contributions (Tier 1-4 descriptions, learning labels)
3. Cite Levin et al. (2012) when referencing super-wicked problems concept
4. Maintain this attribution section if redistributing
5. Not claim proprietary components as original work

---

## 2. Restricted Uses

### 2.1 Commercial Use
❌ **PROHIBITED** without explicit written license:
- Offering consulting services based on this Framework
- Incorporating Framework into commercial products or services
- Training programs or workshops using this Framework
- Selling or licensing derivatives of this Framework
- Using Framework methodologies in commercial contexts

**For Commercial Licensing Inquiries:**  
Contact: Reuben Bowlby, Chief Engineer, HUMMBL Systems LLC  
GitLab: https://gitlab.com/hummbl-dev-group/hummbl-dev-project

### 2.2 Redistribution
❌ **PROHIBITED** without authorization:
- Republishing the Framework without attribution
- Creating derivative works for distribution
- Removing or modifying copyright notices
- Claiming authorship of HUMMBL proprietary components

### 2.3 Misrepresentation
❌ **STRICTLY PROHIBITED:**
- Claiming proprietary components as original work
- Misrepresenting the novelty of concepts with prior art
- Removing attribution to DeepSeek AI or academic sources
- Falsely implying endorsement by HUMMBL Systems

---

## 3. Proprietary Components

The following elements are proprietary intellectual property of HUMMBL Systems LLC and may not be reproduced, modified, or distributed without explicit written permission:

### 3.1 HUMMBL Proprietary Innovations
- **Tier 5 (Super-Wicked) Operationalization:** Quantitative criteria, assessment protocols, and application methodologies
- **5-Question Wickedness Scoring Methodology:** Complete rubric, scoring algorithms, and tier classification formulas
- **Base-N Architecture:** Mathematical foundations, coverage optimization models, and selection algorithms
- **Priority Model System:** Classification algorithms, learning progression optimization, and ROI analysis
- **BASE120 Validation Framework:** All validation data, test results, empirical evidence, and quality metrics
- **Implementation Protocols:** Assessment tools, tier classification algorithms, and organizational readiness frameworks
- **Automated Tooling:** CI/CD validation systems, quality checks, and drift detection mechanisms

### 3.2 Collaborative Contributions (Acknowledged)
The following contributions are acknowledged and attributed:
- **DeepSeek AI (October 2025):** Tier 1-4 descriptive language, learning progression labels, pedagogical insights
- **Academic Literature:** Super-wicked problems concept (Levin et al., 2012), wicked problems concept (Rittel & Webber, 1973)

---

## 4. Academic Citation Requirements

### 4.1 Required Citations

When referencing this Framework in academic work, you MUST include:

**For the Framework:**
```
Bowlby, R. (2025). HUMMBL Unified Tier Framework v1.0: Integrating Problem Complexity, 
Learning Progression, and Base-N Architecture. HUMMBL Systems LLC.
```

**For Super-Wicked Problems Concept:**
```
Levin, K., Cashore, B., Bernstein, S., & Auld, G. (2012). Overcoming the tragedy of 
super wicked problems: constraining our future selves to ameliorate global climate change. 
Policy Sciences, 45(2), 123-152.
```

**For Wicked Problems Concept:**
```
Rittel, H. W. J., & Webber, M. M. (1973). Dilemmas in a general theory of planning. 
Policy Sciences, 4(2), 155-169.
```

### 4.2 Attribution in Derivatives

Any derivative works must:
1. Clearly distinguish between prior art and HUMMBL innovations
2. Maintain all required citations
3. Not claim HUMMBL proprietary components as original contributions
4. Include this license in full if redistributing

---

## 5. Warranty Disclaimer

**THE FRAMEWORK IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND.**

HUMMBL Systems LLC makes no warranties, express or implied, including but not limited to:
- Merchantability or fitness for a particular purpose
- Accuracy or completeness of information
- Suitability for specific problem domains
- Results or outcomes from Framework application

---

## 6. Limitation of Liability

**IN NO EVENT SHALL HUMMBL SYSTEMS LLC BE LIABLE FOR:**
- Direct, indirect, incidental, or consequential damages
- Loss of profits, data, or business opportunities
- Damages arising from use or inability to use the Framework
- Claims by third parties

**Maximum liability is limited to the amount paid for commercial licenses (if any).**

---

## 7. Intellectual Property Protection

### 7.1 Copyright
All content, methodologies, and documentation are protected by copyright law. Unauthorized copying, modification, or distribution is prohibited.

### 7.2 Trademarks
"HUMMBL," "BASE120," "Base-N Architecture," and related marks are trademarks of HUMMBL Systems LLC.

### 7.3 Trade Secrets
Certain methodologies, algorithms, and validation data constitute trade secrets and are protected accordingly.

---

## 8. Termination

This license automatically terminates if you:
- Violate any terms of this license
- Use the Framework for unauthorized commercial purposes
- Fail to provide required attributions
- Misrepresent authorship or novelty

Upon termination, you must:
- Cease all use of the Framework
- Destroy all copies in your possession
- Remove any published derivatives

---

## 9. Governing Law

This license is governed by the laws of the jurisdiction where HUMMBL Systems LLC is registered, without regard to conflict of law principles.

---

## 10. Contact Information

**For Licensing Inquiries:**  
HUMMBL Systems LLC  
Chief Engineer: Reuben Bowlby  
GitLab: https://gitlab.com/hummbl-dev-group/hummbl-dev-project

**For Permission Requests:**  
Submit detailed requests via GitLab issues with:
- Intended use case
- Scope of distribution
- Attribution plan
- Commercial vs. non-commercial designation

---

## 11. License Updates

HUMMBL Systems LLC reserves the right to update these license terms. Continued use of the Framework after updates constitutes acceptance of revised terms.

**Current Version:** 1.0  
**Effective Date:** November 1, 2025  
**Last Updated:** November 1, 2025

---

**By using this Framework, you acknowledge that you have read, understood, and agree to be bound by these license terms.**

---

**Copyright © 2025 HUMMBL Systems LLC. All Rights Reserved.**
